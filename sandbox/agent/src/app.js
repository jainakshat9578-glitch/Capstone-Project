...
// Handle data coming from the pseudo-terminal
 ...
 * @description Reads the content of all files requested in the query parameter 'files' and return their content in json objects. 
...
app.get("/read-files", async (req,res)=>{
     ...
     const results = await Promise.all(fileList.map(async (file)=>{
        ...
        try{
            ...
            return {
                [ filePath.replace(WORKING_DIR, '') ]: `Error redaing file: ${err.message}`,
            }
        }
     }));
...
app.post("/create-files", async (req,res)=>{
    ...
    const results = await Promise.all(files.map(async (fileObj)=>{
        ...
        try{
            ...
            return {
                [filePath]: `Error creating files: ${err.message}`,
            }
        }
    }))
